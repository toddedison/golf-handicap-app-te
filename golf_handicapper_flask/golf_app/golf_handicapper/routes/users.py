...
def _save_picture(file_storage):
    ...
    file_storage.save(os.path.join(dest, fname))
    return fname
...
def index():
    ...
    pag  = User.query.order_by(User.created_at).paginate(
               page=page,
               per_page=current_app.config.get('USERS_PER_PAGE', 30),
               error_out=False)
    return render_template('users/index.html', pagination=pag)
...
def show(user_id):
    ...
    if request.args.get('format') == 'pdf':
        ...
        pdf = generate_scorecard_pdf(user)
...
def edit(user_id):
    ...
    if form.validate_on_submit():
        ...
        if pic and pic.filename and pic.filename.rsplit('.', 1)[-1].lower() in ALLOWED:
            user.picture = _save_picture(pic)
        db.session.commit()
